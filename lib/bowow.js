...
function build$(session) {
  ...
  $.wait = (ms, predicate, interval = 1000) => {
    const throwOnTimeout = typeof predicate === 'function'
    if (!throwOnTimeout) predicate = () => false
    ...
    do {
      ...
      result = session.execute(
        ({ timeout, predicate }) =>
          eval('(' + predicate + ')()') || // eslint-disable-line
          new Promise(resolve => setTimeout(() => resolve(), timeout)),
        { timeout: Math.min(interval, ms), predicate: predicate.toString() }
      )
      ms -= Date.now() - start
    } while (!result && ms > 0)

    if (throwOnTimeout && !result)
      throw new Error(
        'Timeout waiting for predicate to return true: ' + predicate.toString()
      )
