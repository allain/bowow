...
function build$(session) {
  ...
  const $ = function (target) {
    ...
    const iframeSelectorParts = target.match(/^(.*\si?frame\b\S+)\s+(.*)$/)
    if (iframeSelectorParts) {
      lastFramed = true
      const frameSelector = iframeSelectorParts[1]
      const childSelector = iframeSelectorParts[2]
      ...
      switch (matchingFrames.length) {
        ...
        case 1:
          session.execute(({ frameSelector }) => new Promise(resolve => {
          ...
          break;
        default:
          throw new Error('too many frames match selector: ' + frameSelector)
      }
